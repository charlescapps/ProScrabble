...
public class FastDict implements java.io.Serializable {
	...
	private final StringBucket[] hashDict;
	private final int NUM_WORDS; 
	private final static char COMMENT_CHAR = '#';

	public FastDict(BufferedReader dictFile) throws IOException {
		...
		while ((line = dictFile.readLine()) != null) {
			...
			hashDict[hashVal] = new StringBucket(line, hashDict[hashVal]); 
		}
		dictFile.close(); 
	...
	public boolean inDict(String s) {
		StringBucket matches = getMatches(s); 
		if (matches == null) {
			return false; 
		}
	...
	public StringBucket getMatches(String s) {
	...
	public void addToDict(String s) {
		...
		s = s.toUpperCase(); 
		int sHash = hash(s); 
		hashDict[sHash] = new StringBucket(s,hashDict[sHash]); 
